...
export function AccessRow({
  ...
  return (
    <section
      ...
      className={cn(
        'space-y-2.5 border-b border-[var(--color-line)] py-4 transition-opacity last:border-b-0',
        disabled && 'opacity-50',
      )}
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          <span
            aria-hidden="true"
            className="flex size-7 shrink-0 items-center justify-center rounded-md bg-[var(--color-brand-tint)]/60 text-[var(--color-brand-700)]"
          >
            <i className={cn(icon, 'text-base')} />
          </span>
          <h3 className="text-[length:var(--text-sm)] font-bold tracking-[-0.005em] text-[var(--color-ink)]">
            {title}
          </h3>
          {showCount ? (
...
export function AccessChip({
  ...
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--color-brand-tint-2)] bg-[var(--color-brand-tint)] py-1 pl-3 pr-1.5 text-[length:var(--text-xs)] font-semibold text-[var(--color-brand-700)]">
      ...
      <button
        ...
        disabled={disabled}
        aria-label={label}
        className="flex size-4 shrink-0 items-center justify-center rounded-full text-[var(--color-brand-700)]/70 transition-colors hover:bg-[var(--color-brand-600)] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-brand-600)]/40 disabled:cursor-not-allowed"
      >
